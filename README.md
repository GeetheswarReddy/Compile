...
Review the change set and approve the update. The template adds a quota table and a private execution Lambda in a VPC with no internet route, no DNS, and a deny-all network ACL. The executor has no database, S3, or Bedrock permissions. API and verification functions invoke it privately; learner code does not run inside their privileged processes. CPU, memory, file-size, process-count, environment, descriptor, import, and file-open restrictions apply in the child. Outside Lambda, failure to install the additional syscall filter fails closed; Lambda rejects that nested filter, so the Lambda-only fallback relies on the private function/VPC boundary plus the remaining child restrictions. This is a bounded anonymous demo, not an authenticated production learning service.
